...
Có thể bạn đã từng dùng Microsoft Word để gõ bài Toán và xoay sở khó khăn với Equation Editor hay MathType, hoặc đã nghe đến LaTeX nhưng bị đống lệnh phức tạp làm nản lòng. Typst là lựa chọn thứ ba — giữ lại chất lượng của LaTeX
...
Dự án Typst bắt đầu vào năm 2019 tại *ETH Zürich* (Viện Công nghệ Liên bang Thụy Sĩ), một trong những viện nghiên cứu kỹ thuật hàng đầu thế giới. Nghiên cứu sinh *Laurenz Mädje*, dưới sự hướng dẫn của các giáo sư tại ETH, đã bắt đầu xây dựng một hệ thống soạn thảo tài liệu mới với ba mục tiêu cốt lõi:
...
*1. Typst CLI (Command Line Interface)* — công cụ dòng lệnh, cài đặt trên máy tính cá nhân. Biên dịch file `.typ` thành PDF hoặc PNG. Phù hợp với người dùng chuyên nghiệp,
có thể tích hợp vào CI/CD pipeline và script tự động hóa.

*2. Typst App (Web Editor)* — truy cập tại _typst.app_. Trình soạn thảo trực tuyến, không cần cài đặt gì. Hỗ trợ cộng tác thời gian thực, lưu trữ đám mây, chia sẻ qua link.

*3. Typst Universe (Package Registry)* — kho gói mở rộng chính thức tại _typst.app/universe_. Tương tự CTAN của LaTeX hay npm của JavaScript. Bao gồm các gói: `cetz` (vẽ hình),
`polylux` (slide trình chiếu), `showybox` (hộp định lý, ví dụ), `equate` (đánh số công thức linh hoạt), và hàng trăm gói do cộng đồng đóng góp.
...
Có hai cách sử dụng Typst: cài đặt trực tiếp trên máy tính (khuyên dùng cho công việc thường xuyên và tích hợp với VS Code), hoặc dùng Typst App trên trình duyệt (như HeyTeX, không cần cài đặt gì, phù hợp khi muốn thử nhanh).
...
#figure(
  image("../assets/figures/VSCode+Tinymist_Typst.png", width: 80%),caption:[VS Code với extension Tinymist hỗ trợ soạn thảo Typst])


#chu-y[
  Sau khi cài Tinymist, nhấn `Ctrl+Shift+P` (Windows/Linux) hoặc `Cmd+Shift+P` (macOS) và gõ "Typst Preview" để mở cửa sổ xem trước PDF bên cạnh editor. Mỗi lần bạn lưu file (`Ctrl+S` / `Cmd+S`), preview sẽ cập nhật ngay lập tức.
]
...
#figure(
  image("../assets/figures/NewProject_Typst_HeyTeX.png", width: 70%), caption:[Tạo dự án Typst mới trên HeyTeX])
...
#chu-y[
  ...
  *Nhược điểm:* Cần kết nối Internet. Thông thường, HeyTeX có giới hạn số dự án ở tài khoản miễn phí, và có thể không phù hợp với dự án quá lớn.
...
Typst sử dụng kỹ thuật biên dịch tăng dần (*incremental compilation*): khi bạn thay đổi một phần nhỏ, chỉ phần đó và các phần liên quan được biên dịch lại. Với tài liệu nhỏ đến trung bình, thời gian biên dịch thường *dưới 1 giây*,
